...
T0=np.nansum(regridT0*A_weighted_global) #Reference temperature
...
#Equations
Pterm=(co2/C0)**0.5 #CO2 term
Tterm=((np.exp((((-Ea*1000)/Rc)*(1/regridT-1/T0))))) #Temp Term
Tterm[Tterm<0.000001]=0 #To remove ice and glaciers
Rterm=R*bsil #Runoff term
F=Pterm*Tterm*Rterm #Weathering flux, t/yr/m^2
F[F<0]=0
...
#Plotting
F_copy=np.copy(F)
F_copy[F!=0]=np.nan #Is used to set all non zero values nan which is used to plot a 0 weathering flux field
...
plt.title('CMIP-6 Control Weathering with Lithology')
...
plt.show()
plt.savefig('CMIPControlMap.png')
...
np.save('LithW',F) #Save weathering Array
