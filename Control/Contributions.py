...
T0=287.1968543493361 #From piControl run
#Define variables
...
F_copy=np.copy(Rterm)
F_copy[Rterm!=0]=np.nan
...
fig=plt.contourf(lon1,lat1,Rterm,norm=colors.LogNorm(),levels=np.logspace(-3.5,np.log10(np.nanmax(Rterm)+1),1000),cmap='Blues')
plt.title('CESM Control Runoff Term')
cbar=plt.colorbar(fig)
cbar.set_label('Runoff Term',fontsize=20)
cbar.set_ticks([1E-4,1E-3,5E-3,1E-2,5E-2,1E-1,5E-1,1,5,1E+1,50,1E+2])
plt.savefig('CMIPRunoff.png')

#------------------------------------------------------------------------------#
#Plotting Temperature
#Is used to set all non zero values nan which is used to plot a 0 weathering flux field
F_copy=np.copy(Tterm)
F_copy[Tterm!=0]=np.nan

#plotting the 0 value weathering fluxes
fig=plt.contourf(lon1,lat1,F_copy,colors='silver')
#Plotting the main data with a logarithmic colorbar
fig=plt.contourf(lon1,lat1,Tterm,norm=colors.LogNorm(),levels=np.logspace(-3.5,np.log10(np.nanmax(Tterm)+1),1000),cmap='Blues')
plt.title('CESM Control Temperature Term')
cbar=plt.colorbar(fig)
cbar.set_label('Temperature Term',fontsize=20)
cbar.set_ticks([1E-4,1E-3,5E-3,1E-2,5E-2,1E-1,5E-1,1,5,1E+1,50,1E+2])
plt.savefig('CMIPTemp.png')
