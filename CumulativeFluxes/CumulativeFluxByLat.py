...
LandArea=A*Land #Gives the area of land in each grid box
CumulativeLand=np.sum(LandArea*A,axis=1) #zonal mean
...
CMIP1=np.cumsum(np.flip(Control[0:180])+Control[180:])/np.sum(Control)*100
CMIP2=np.cumsum(np.flip(two[0:180])+two[180:])/np.sum(two)*100
CMIP4=np.cumsum(np.flip(four[0:180])+four[180:])/np.sum(four)*100

#Save arrays for future plots
np.save('CMIP1lat.npy',CMIP1)
np.save('CMIP2lat.npy',CMIP2)
np.save('CMIP4lat.npy',CMIP4)
...
#Cumulative land per latitude
plt.plot(np.flip(lat1[0:180]),np.cumsum(np.flip(CumulativeLand[0:180])+CumulativeLand[180:])/np.sum(CumulativeLand)*100,':m',label='Land')
